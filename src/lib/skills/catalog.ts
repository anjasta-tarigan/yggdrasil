...
/**
 * Re-exported from `token-estimator.ts` so that client components importing
 * `estimateTokens` do not transitively pull in `@/db` (better-sqlite3 /
 * sqlite-vec) through `catalog.ts`.
 */
