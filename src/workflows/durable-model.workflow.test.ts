 ...
 *
 * The probe passes an unconfigured provider id, so the registry resolution fails
...
describe("DurableLanguageModel across the step boundary", () => {
  it("deserializes inside a step", async () => {
    const run = await start(durableModelProbeWorkflow, [
      { providerId: "p", modelId: "m", baseUrl: "", apiKey: "" },
    ]);
    ...
    // The registry resolution is expected to fail (no such provider), and the
    // failure must come from inside the step — proving the step ran.
    expect(result.resolveError).toMatch(/not in the registry/);
  });
});
